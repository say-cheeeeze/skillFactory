 ...
 * 2 초 (추가 시간 없음)	128 MB	129814	67318	52427	50.834%
 * 문제
...
public class Q2164 {
	...
	private static void printLastQueue( int num ) {
		...
		while ( true ) {
			...
			// 2. 만약 1개 남았으면 종료함..
			if ( cardQueue.size() < 2 ) {
				answer = cardQueue.peek().intValue();
				break;
			}
			// 1. 하나를 버립니다.
			cardQueue.poll();
			...
			// 3. 하나를 더 뽑고 이번엔 맨 뒤에 붙인다.
			Integer head = cardQueue.poll();
			cardQueue.add( head );
		}
