 ...
 * 2 초 (추가 시간 없음)	1024 MB (추가 메모리 없음)	23864	12882	11142	55.117%
 * 문제
...
public class isPalindrome {
	...
	public static int isPalindrome( String s ) {
		int acc = 1;
		return recursion( s, 0, s.length() - 1, acc );
	}
	
	public static int recursion( String s, int l, int r, int acc ) {
		if ( l >= r ) {
			System.out.println( 1 + " " + acc );
			return 1;
		}
		else if ( s.charAt( l ) != s.charAt( r ) ) {
			System.out.println( 0 + " " + acc );
			return 0;
		}
		else {
			return recursion( s, l + 1, r - 1, ++acc );
		}
	}
}
